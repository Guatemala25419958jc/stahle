...
export default function AdminPage() {
  ...
  const addProduct = async (event: React.FormEvent) => { event.preventDefault(); const uploaded:string[]=[]; for (const file of imageFiles) { const form=new FormData(); form.append("file",file); const upload=await fetch("/api/admin/images",{method:"POST",body:form}); if(upload.ok){ const item=await upload.json(); uploaded.push(item.url); } } const response = await fetch("/api/admin/products", {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({name:productName, collection:productCollection, room:productRoom, images:[...uploaded,...productImages.split(",").map(x=>x.trim()).filter(Boolean)]})}); if(response.ok){ const item=await response.json(); setProducts([...products,{id:item.id,name:productName,collection:productCollection,room:productRoom}]); setProductName(""); } };
